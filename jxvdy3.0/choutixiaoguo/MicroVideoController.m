...
@interface MicroVideoController ()<UITableViewDataSource,UITableViewDelegate>
{
...
@implementation MicroVideoController
...
-(void)loadDataPage {
    ...
    NSString *fpath =[LZXHelper getFullPathWithMyCacheFile:@"cahcahe1"];
    BOOL isExit =[[NSFileManager  defaultManager] fileExistsAtPath:fpath];
    BOOL  isTimeOut=[LZXHelper isTimeOutFile:@"cachcahe1" time:60*60];
    
    if (isExit==YES&&isTimeOut==NO) {
        NSData *data =[NSData dataWithContentsOfFile:[LZXHelper getFullPathWithMyCacheFile:@"cahcahel"]];
        NSArray *arr =[NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingMutableContainers error:nil];
        
        [self.dataArr removeAllObjects];
        for (NSDictionary *videoDict in arr) {
            MicroVideoModel *model=[[MicroVideoModel alloc] init];
            model.videoImg =videoDict[@"img"];
            model.videoTitle=videoDict[@"title"];
            model.videoId =videoDict[@"id"];
            
            [self.dataArr addObject:model];

        }
        [_videoTableView reloadData];
        [MMProgressHUD  dismissWithError:@"恭喜" title:@"本地下载完成"];
        return;
    ...
    [_manager  GET:str parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        [wekSelf.dataArr removeAllObjects];
        NSData *data =(NSData *)responseObject;
        NSString *filePath =[LZXHelper getFullPathWithMyCacheFile:@"cahcahel"];
        ...
        for (NSDictionary *videoDict in jsonArr) {
            MicroVideoModel *model=[[MicroVideoModel alloc] init];
            model.videoImg =videoDict[@"img"];
            model.videoTitle=videoDict[@"title"];
            model.videoId =videoDict[@"id"];
            
            [wekSelf.dataArr addObject:model];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(nonnull NSIndexPath *)indexPath {
    ...
    detailVC.movieID=model.videoId;
    NSLog(@"model.movieID:%@",model.videoId);
...
//        cell = [[VideoTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellName];
    ...
    MicroVideoModel *model=self.dataArr[indexPath.row];
...
-(void)creatShareView {
    __weak  MicroVideoController *weakSelf=self;
